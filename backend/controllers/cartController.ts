...
const updateCartItem = async (req: Request, res: Response): Promise<any> => {
    const transaction = await sequelize.transaction();
...
const removeFromCart = async (req: Request, res: Response): Promise<any> => {
    const transaction = await sequelize.transaction();
    ...
    try {
        ...
        const totalAmount = remainingItems.reduce((sum: number, item: any) => {
            return sum + (parseFloat(item.price) * item.quantity);
        }, 0);
...
// Clear Cart
const clearCart = async (req: Request, res: Response): Promise<any> => {
    const transaction = await sequelize.transaction();
